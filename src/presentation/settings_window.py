...
class SettingsWindow(QWidget):
    ...
    def _create_api_section(self) -> QWidget:
        ...
        max_tokens_layout.setSpacing(6)
        
        max_tokens_label = QLabel("Response Length (Max Tokens):")
        max_tokens_label.setStyleSheet(f"""
            QLabel {{
                font-size: 13px;
                font-weight: 500;
                color: {COLORS["text_primary"]};
        ...
        max_tokens_input_layout = QHBoxLayout()
        max_tokens_input_layout.setSpacing(8)
        
        self.max_tokens_input = QLineEdit()
        self.max_tokens_input.setPlaceholderText("250")
        self.max_tokens_input.setStyleSheet(f"""
            QLineEdit {{
                background: white;
                border: 1px solid {COLORS["border_light"]};
                border-radius: 8px;
        ...
        """)
        max_tokens_input_layout.addWidget(self.max_tokens_input)
        
        hint_label = QLabel("(50-500, lower = shorter responses)")
        ...
        """)
        max_tokens_input_layout.addWidget(hint_label)
        max_tokens_input_layout.addStretch()
        
        max_tokens_layout.addLayout(max_tokens_input_layout)
    ...
    def _create_character_section(self) -> QWidget:
        ...
        for field_key, field_label, field_hint in field_definitions:
            ...
            # Use minimum height instead of maximum, allow expansion for longer content
            field_input.setMinimumHeight(80)
            # For personality field, allow more height
            if field_key == "personality":
                field_input.setMinimumHeight(120)
    ...
    def _load_settings(self):
        ...
        for field_key, field_input in self.character_fields.items():
            if field_key in character_config and character_config[field_key]:
                # Ensure we set the full text content
                content = character_config[field_key]
                if content:  # Only set if content is not empty
                    field_input.setPlainText(content)
    ...
    def _save_settings(self):
        ...
        try:
            ...
            # Validate range (50-500)
            if 50 <= max_tokens <= 500:
                self.config_manager.set_max_tokens(max_tokens)
            else:
                # Use default if out of range
                self.config_manager.set_max_tokens(250)
        except ValueError:
